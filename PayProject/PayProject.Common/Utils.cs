...
namespace PayProject.Common
{
    public class Utils
    {
        ...
        public static List<Guid> StrToListGuid(string str)
        {
            var list = new List<Guid>();
            if (!str.Contains(","))
            {
                list.Add(Guid.Parse(str));
            }
            var slist = str.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var item in slist)
            {
        ...
        /// <param name="len">长度</param>
        /// <returns></returns>
        public static string GetRandNumberCode(int len)
        {
        ...
        public static string GetCurToken()
        {
            ...
            var tokenHeader = _context.HttpContext.Request.Headers["Authorization"];
        ...
        #endregion
        #region 十进制TO34进制
        const string X34 = "ZPQJR8SLV63MYK4NE1WBX5GI7F9DACTUH2";
        public static string BigIntTo34HexStr(int val)
        {
            string Result = "";
            while (val >= 34)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public static int CurrentDayTimeStamp()
        {
            return Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd")).ToTimeStamp();
        }
        public static string Pjax()
        {
            HttpContextAccessor _context = new HttpContextAccessor();
