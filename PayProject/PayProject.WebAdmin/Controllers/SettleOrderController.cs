...
namespace PayProject.WebAdmin.Controllers
{
    ...
    [Route("settleorder")]
    [Authorize]
    public class SettleOrderController : Controller
    {
        [HttpGet("index")]
        ...
        {
            SettleOrder model = SettleOrderBll._.GetModelAsync(d => d.Id == id).Result.data;
            return View(model);
        }

        [HttpGet("/api/settleorder/getpages")]
        ...
        [HttpPost("/api/settleorder/sendnotify")]
        ...
        [HttpPost("/api/settleorder/replaynotify")]
        public async Task<ApiResult<string>> ReplayNotify(string parm)
        {
            ...
            {
                SettleOrderBll._.SendNotify(item.SqlFilters(), false, true);
            }
            return await Task.Run(() => res);
        }

        [HttpPost("/api/settleorder/query")]
        ...
        [HttpPost("/api/settleorder/batchquery")]
        public async Task<ApiResult<string>> BatchQuery(string parm)
        {
            ...
            foreach (var item in list)
            {
                SettleOrderBll._.Query(WebConfig.MchId, item.SqlFilters());
            }
            return await Task.Run(() => res);
        }
    }
