...
{
    public class Settle_OrderBll
    {
        private static readonly Settle_OrderBll instance = new Settle_OrderBll();

        // 显示的static 构造函数
        //没必要标记类型 - 在field初始化以前
        static Settle_OrderBll()
        {
        }

        private Settle_OrderBll()
        {
        }

        public static Settle_OrderBll Instance
        {
            get
            {
                return instance;
        ...
        public async Task<QueryReturn> Query(string mchid, string orderid )
        {
            ...
            Settle_order order = DB.Context.From<Settle_order>().Where(p => p.Order_id == orderid).ToFirstDefault();
            ...
            else
            {
                ...
                Settle_mch mch = OnlineSettle.GetMch(order.Mch_id);
                ...
                Settle_plat plat = OnlineSettle.GetPlat(mch.Plat_id);
                if (plat == null)
                {
        ...
        {
            Settle_plat p = OnlineSettle.GetPlat(pid);
            Settle_mch m = OnlineSettle.GetMch(mid);
            OnlineSettle onlinepay = (OnlineSettle)Activator.CreateInstance(Type.GetType(p.Plat_class), p, m);
            ...
            if (r.IsCheck)
            {
                if (r.IsPay)
                {
                    Settle_order order = DB.Context.From<Settle_order>().Where(o => o.Order_id == r.OrderNumber).ToFirstDefault();
        ...
        public void Publish(string mchid,string orderid,int ttl=0) {
            var msg = new SettleOrderNotify { mchid = mchid, orderid = orderid };
            if (ttl <= 0)
            {
